...
class ObjectBrowserSourcePhysicalFileItem extends ObjectBrowserItem implements SourcePhysicalFileItem {
  ...
  constructor(parent: ObjectBrowserFilterItem, readonly sourceFile: IBMiObject) {
    ...
    this.tooltip = `Member quantity:\t${sourceFile.memberCount}`;
...
export function initializeObjectBrowser(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    ...
    ,
    // vscode.commands.registerCommand(`Hawkeye-Pathfinder.searchSourceFiles-a`, async (node) => {
    //   const parameters = {
    //     path: node?.path,
    //     filter: node?.filter
    //   }
    ...
    //   }
    // }),
    // vscode.commands.registerCommand(`Hawkeye-Pathfinder.displayFileSetsUsed-a`, async (node) => {
    //   const parameters = {
    //     path: node?.path,
    //     filter: node?.filter
    //   }
    ...
    //               // Hawkeye-Pathfinder-DSPSCNSRC
    //               // returns results member name with member type as extension
    //               let results = await Search.HwkdisplayFileSetsUsed(instance, pathParts[0], pathParts[1], searchTerm, parameters?.filter?.protected);
